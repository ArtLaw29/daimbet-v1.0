...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "send_report") {
      // Generate report before nuclear reset
      ...
      if (resendApiKey) {
        const emailRes = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            html: `<h2>Rapport pré-réinitialisation totale</h2><p>Date : ${report.generated_at}</p><p>Ce rapport contient l'intégralité des données avant suppression.</p><pre style="font-size:11px;max-height:600px;overflow:auto;">${JSON.stringify(report.data, null, 2).substring(0, 50000)}</pre>`,
          }),
        });
    ...
    if (action === "execute_reset") {
      ...
      // Delete non-admin profiles and user_roles
