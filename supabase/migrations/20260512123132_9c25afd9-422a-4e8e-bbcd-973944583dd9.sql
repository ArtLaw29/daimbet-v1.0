...
BEGIN
  ...
  v_rewards := COALESCE(v_content.data->('rewards_' || v_variant), '[500,300,200,100,50]'::jsonb);

  -- Only Top 5 are rewarded; rank 6+ earns 0 DC.
  IF jsonb_typeof(v_rewards) = 'array' AND v_rank <= LEAST(5, jsonb_array_length(v_rewards)) THEN
